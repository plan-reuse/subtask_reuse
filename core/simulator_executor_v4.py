...
class SimulatorExecutor:
    ...
    def __init__(self):
        ...
        self.stovetop_cookware_position = (1.3, 8.5, 1.0)  # Cookware on stove
    ...
    def _get_object_body_id(self, obj_name):
        """Helper to get body ID for an object from either base or extended objects"""
        if obj_name in self.movables:
            return self.movables[obj_name]
        elif obj_name in self.extended_objects:
            return self.extended_objects[obj_name]
        else:
            # Try to find by world lookup
            try:
                return self.world.name_to_body(obj_name)
            except:
                print(f"Object {obj_name} not found in any object dictionary")
                return None
    ...
    def _attach_object_to_robot(self, obj_name):
        """Attach object to robot using VLM-TAMP style geometric transforms"""
        body_id = self._get_object_body_id(obj_name)
        if body_id is None:
            print(f"Cannot attach {obj_name} - object not found")
            return False
        ...
        try:
            ...
            # grasp_pose = multiply(invert(gripper_pose), object_pose)
            ...
            print(f"    Moved {obj_name} to gripper position: {object_position[:3]}")
            
            # Step 5: Recalculate grasp pose with new position
            object_pose = get_pose(body_id)
            self.grasp_pose = multiply(invert(gripper_pose), object_pose)
            
            print(f"✅ Attached {obj_name} to robot gripper with transform")
            return True
            
        except Exception as e:
            print(f"❌ Failed to attach {obj_name}: {e}")
            import traceback
            traceback.print_exc()
            return False
    ...
    def _place_object_in_region(self, obj_name, region):
        """Place object in appropriate region location"""
        body_id = self._get_object_body_id(obj_name)
        if body_id is None:
            print(f"Cannot place {obj_name} - object not found")
            return False
        ...
        new_pose = Pose(point=Point(*position))
        set_pose(body_id, new_pose)
        print(f"✅ Placed {obj_name} in {region} at position {position}")
        return True
    
    def _place_object_in_container(self, obj_name, container):
        ...
        new_pose = Pose(point=Point(*position))
        set_pose(body_id, new_pose)
        print(f"✅ Placed {obj_name} in {container} at position {position}")
        return True
    ...
    def _execute_take_food_from_cookware(self, action: str, verbose: bool) -> bool:
        ...
        if verbose:
            print(f"🍲 Taking {food} from {cookware} at {region}")
        ...
        print(f"  Step 2: Reaching into {cookware}...")
        grasp_pose = [0.3, 0.1, -0.3, -1.4, 0.0, -1.0, 0.0]
        set_joint_positions(self.robot_body, self.left_joints, grasp_pose)
        wait_for_duration(0.5)
        
        # Step 3: Attach food to robot
        print(f"  Step 3: Grasping {food}...")
        if self._attach_object_to_robot(food):
            self.holding_object = food
        else:
            return False
            
        # Step 4: Lift food from cookware
        print(f"  Step 4: Lifting {food} from {cookware}...")
        set_joint_positions(self.robot_body, self.left_joints, TOP_HOLDING_LEFT_ARM)
        wait_for_duration(0.5)
        
        print(f"✅ Successfully took {food} from {cookware}")
        return True
    ...
    def _execute_place_food_on_utensil(self, action: str, verbose: bool) -> bool:
        ...
        robot, food, utensil, region = parts[1], parts[2], parts[3], parts[4]
        
        if verbose:
            print(f"🍽️ Placing {food} on {utensil} at {region}")
        ...
        print(f"  Step 2: Positioning over {utensil}...")
        ...
        print(f"  Step 3: Releasing {food}...")
        self._detach_object_from_robot()
        
        # Step 4: Place food on utensil
        print(f"  Step 4: Placing {food} on {utensil}...")
        if not self._place_food_on_utensil_position(food, utensil, region):
            return False
        ...
        self.holding_object = None
        print(f"✅ Successfully placed {food} on {utensil}")
        return True
    ...
    def _execute_take_food_from_utensil(self, action: str, verbose: bool) -> bool:
        ...
        if verbose:
            print(f"🍽️ Taking {food} from {utensil} at {region}")
        ...
        print(f"  Step 2: Reaching to {utensil}...")
        ...
        print(f"  Step 4: Lifting {food} from {utensil}...")
        set_joint_positions(self.robot_body, self.left_joints, TOP_HOLDING_LEFT_ARM)
        wait_for_duration(0.5)
        
        print(f"✅ Successfully took {food} from {utensil}")
        return True
    ...
    def _execute_remove_cookware_from_stovetop(self, action: str, verbose: bool) -> bool:
        ...
        if verbose:
            print(f"🔥 Removing {cookware} from {stovetop}")
        ...
        print(f"  Step 2: Grasping {cookware}...")
        grasp_pose = [0.3, 0.2, -0.2, -1.2, 0.0, -0.8, 0.0]
        set_joint_positions(self.robot_body, self.left_joints, grasp_pose)
        wait_for_duration(0.5)
        
        # Step 3: Attach cookware to robot
        print(f"  Step 3: Lifting {cookware}...")
        if self._attach_object_to_robot(cookware):
            self.holding_object = cookware
        else:
            return False
            
        # Step 4: Lift cookware from stovetop
        print(f"  Step 4: Removing {cookware} from stovetop...")
        set_joint_positions(self.robot_body, self.left_joints, TOP_HOLDING_LEFT_ARM)
        wait_for_duration(0.5)
        
        print(f"✅ Successfully removed {cookware} from stovetop")
        return True
        
    def _place_food_in_cookware(self, food_name, cookware_name, region):
        """Place food inside cookware at specified region"""
        food_id = self._get_object_body_id(food_name)
        cookware_id = self._get_object_body_id(cookware_name)
        
        if food_id is None or cookware_id is None:
            print(f"Cannot place {food_name} in {cookware_name} - objects not found")
            return False
        ...
        print(f"✅ Placed {food_name} inside {cookware_name} at {food_position[:3]}")
        return True
        
    def _place_food_on_utensil_position(self, food_name, utensil_name, region):
        """Place food on utensil at specified region"""
        food_id = self._get_object_body_id(food_name)
        utensil_id = self._get_object_body_id(utensil_name)
        
        if food_id is None or utensil_id is None:
            print(f"Cannot place {food_name} on {utensil_name} - objects not found")
            return False
        ...
        print(f"✅ Placed {food_name} on {utensil_name} at {food_position[:3]}")
        return True
        
    def _place_cookware_on_stovetop_position(self, cookware_name):
        """Place cookware on stovetop at fixed position"""
        cookware_id = self._get_object_body_id(cookware_name)
        if cookware_id is None:
            print(f"Cannot place {cookware_name} - cookware not found")
            return False
            
        # Set cookware position on stovetop
        new_pose = Pose(point=Point(*self.stovetop_cookware_position))
        set_pose(cookware_id, new_pose)
        print(f"✅ Placed {cookware_name} on stovetop at {self.stovetop_cookware_position}")
        return True
    ...
    def _execute_pickup_from_region(self, action: str, verbose: bool) -> bool:
        ...
        print(f"  Step 3: Attaching {obj} to robot gripper...")
        if self._attach_object_to_robot(obj):
            self.holding_object = obj
        else:
            return False
            
        # Step 4: Lift arm with object
        print(f"  Step 4: Lifting {obj}...")
        set_joint_positions(self.robot_body, self.left_joints, TOP_HOLDING_LEFT_ARM)
        wait_for_duration(0.5)
        
        print(f"✅ Successfully picked up {obj} from {region}")
        return True
    
    def _execute_pickup_from_container(self, action: str, verbose: bool) -> bool:
        """Execute pickup_from_container action with enhanced arm stretching and manual attachment"""
        parts = action.split()
        if len(parts) < 4:
            return False
        ...
        # Step 3: Manually attach object to robot
        print(f"  Step 3: Attaching {obj} to robot gripper...")
        if self._attach_object_to_robot(obj):
            self.holding_object = obj
        else:
        ...
        print(f"✅ Successfully picked up {obj} from {container}")
        return True
    ...
    def _execute_putdown_to_region(self, action: str, verbose: bool) -> bool:
        ...
        print(f"  Step 3: Detaching {obj} from robot...")
        self._detach_object_from_robot()
        
        # Step 4: Place object in region
        print(f"  Step 4: Placing {obj} in {region}...")
        if not self._place_object_in_region(obj, region):
            return False
        ...
        self.holding_object = None
        print(f"✅ Successfully put down {obj} in {region}")
        return True
    ...
    def _execute_putdown_to_container(self, action: str, verbose: bool) -> bool:
        ...
        if verbose:
            print(f"🤖 Putting down {obj} to {container}")
        ...
        # Step 4: Manually place object in container
        print(f"  Step 4: Placing {obj} in {container}...")
        if not self._place_object_in_container(obj, container):
            return False
        ...
        self.holding_object = None
        print(f"✅ Successfully put down {obj} in {container}")
        return True
    ...
    def _execute_put_food_in_cookware(self, action: str, verbose: bool) -> bool:
        ...
        if verbose:
            print(f"🍲 Putting {food} into {cookware} at {region}")
        ...
        print(f"  Step 2: Positioning over {cookware}...")
        ...
        # Step 4: Place food inside cookware
        print(f"  Step 4: Placing {food} in {cookware}...")
        if not self._place_food_in_cookware(food, cookware, region):
            return False
        ...
        self.holding_object = None
        print(f"✅ Successfully put {food} into {cookware}")
        return True
    ...
    def _execute_place_cookware_on_stovetop(self, action: str, verbose: bool) -> bool:
        ...
        print(f"  Step 2: Lowering {cookware} to stovetop...")
        place_pose = [0.4, 0.3, -0.2, -1.1, 0.0, -0.7, 0.0]
        set_joint_positions(self.robot_body, self.left_joints, place_pose)
        wait_for_duration(0.5)
        
        # Step 3: Detach cookware from robot
        print(f"  Step 3: Releasing {cookware}...")
        self._detach_object_from_robot()
        
        # Step 4: Place cookware on stovetop
        print(f"  Step 4: Positioning {cookware} on stovetop...")
        if not self._place_cookware_on_stovetop_position(cookware):
            return False
        ...
        self.holding_object = None
        print(f"✅ Successfully placed {cookware} on stovetop")
        return True
...
def main():
    ...
    # Test action list - comprehensive manipulation test

    # test_actions = [
    #     "move pr2 kitchen_corner fridge",
    #     "pickup_from_container pr2 chicken fridge",
    #     "move pr2 fridge countertop_2",
    #     "putdown_to_region pr2 chicken countertop_2",
    #     "move pr2 countertop_2 fridge",
    #     "move pr2 fridge countertop_2",
    #     "pickup_from_region pr2 chicken countertop_2",
    ...
    #     "wait_for_food_to_cook stove pot chicken",
    #     "turn_off_stove pr2 stove",
    # ]

    test_actions = [
        "move pr2 kitchen_corner countertop_2",
        "pickup_from_region pr2 braiserbody countertop_2",
        "move pr2 countertop_2 stove",
        "place_cookware_on_stovetop pr2 braiserbody stove",
    ]

    
    try:
        print("=== TESTING SIMULATOR EXECUTOR V4 ===")
