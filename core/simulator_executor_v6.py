"""Simulator executor that maps PDDL actions to PyBullet robot commands with improved grasping using proper IK."""
...
from pybullet_tools.utils import (
    ...
    body_from_end_effector, get_aabb, get_aabb_center, get_joint_positions
)
from pybullet_tools.pr2_utils import (
    PR2_GROUPS, SIDE_HOLDING_LEFT_ARM, TOP_HOLDING_LEFT_ARM,
    rightarm_from_leftarm, REST_LEFT_ARM, open_arm, LEFT_ARM, get_gripper_link, close_gripper
)
...
class SimulatorExecutor:
    """Executes PDDL action plans in PyBullet simulation with improved IK-based grasping."""
    ...
    def __init__(self):
        ...
        self.region_locations = {
            ...
            'fridge': (1.5, 4.6, 2.3),
            # 'fridge': (1.583, 4.519, 1.143),
            # 'fridge': (1.583, 4.519, 3.1415),
            'sink': (1.5, 6.0, 3.1415),
            'countertop_1': (1.5, 6.8, 3.1415),
            'cabinet': (1.5, 7.6, 3.1415),
    ...
    def _initialize_simulation(self):
        ...
        try:
            ...
            set_camera_pose(camera_point=[3, 6, 2], target_point=[2, 6, 1])
            # set_camera_pose(camera_point=[3, 6, 3], target_point=[2, 6, 1])
    ...
    def _set_enhanced_object_positions(self):
        ...
        EXTENDED_OBJECT_POSITIONS = {
            ...
            'plate': (1.0, 6.0, 1.0),               # In dishwasher
        }
    ...
    def _get_object_pose_for_grasping(self, pddl_obj_name):
        """Get object pose and calculate optimal grasp pose"""
        ...
        object_aabb = get_aabb(body_id)
        object_center = get_aabb_center(object_aabb)
        
        print(f"  Object pose: {object_pose[0][:3]}")
        print(f"  Object center: {object_center[:3]}")
        print(f"  Object AABB: {object_aabb}")
        
        # Get current robot base position to check reachability
        robot_base_pos = get_base_values(self.robot_body)
        distance_to_object = ((object_center[0] - robot_base_pos[0])**2 + 
                             (object_center[1] - robot_base_pos[1])**2)**0.5
        print(f"  Robot base: {robot_base_pos[:2]}, Distance to object: {distance_to_object:.3f}m")
        
        # Calculate grasp pose (approach from a more reachable angle)
        # Position grasp slightly in front of object center towards robot base
        robot_base_pos = get_base_values(self.robot_body)
        
        # Vector from robot to object
        to_object_x = object_center[0] - robot_base_pos[0]
        to_object_y = object_center[1] - robot_base_pos[1]
        
        # Normalize vector and move grasp point slightly towards robot
        distance = (to_object_x**2 + to_object_y**2)**0.5
        if distance > 0:
            offset_x = -0.05 * to_object_x / distance  # 5cm towards robot
            offset_y = -0.05 * to_object_y / distance
        else:
            offset_x, offset_y = 0, 0
        
        grasp_position = (
            object_center[0] + offset_x,
            object_center[1] + offset_y, 
            object_center[2] + 0.05  # 5cm above object center for grasping
        )
        
        # Calculate orientation to point gripper towards object from a reachable angle
        from pybullet_tools.utils import quat_from_euler
        import math
        
        # Angle to point gripper towards object center
        angle_to_object = math.atan2(to_object_y, to_object_x)
        
        # Gripper orientation: slightly tilted downward, facing object
        grasp_orientation = quat_from_euler((math.pi*0.8, 0, angle_to_object))  # 80% downward tilt
        
        grasp_pose = (grasp_position, grasp_orientation)
        print(f"  Calculated grasp pose: position={grasp_position}, orientation={grasp_orientation}")
        return grasp_pose, object_pose

    def _calculate_inverse_kinematics(self, target_pose, arm='left'):
        """Calculate inverse kinematics for target pose using VLM-TAMP approach"""
        try:
            print(f"  Calculating IK for target pose: {target_pose[0][:3]}")
            
            # Set robot to a reasonable configuration before IK
            arm_joints = joints_from_names(self.robot_body, PR2_GROUPS['left_arm'])
            
            # Set default arm configuration (similar to VLM-TAMP approach)
            default_conf = SIDE_HOLDING_LEFT_ARM
            set_joint_positions(self.robot_body, arm_joints, default_conf)
            
            # Open gripper
            open_arm(self.robot_body, arm)
            
            # Custom limits for better IK solutions
            custom_limits = {
                joint: (-3.14, 3.14) for joint in arm_joints  # Allow full range
            }
            
            # First try IKFast if available
            if is_ik_compiled():
                print("  Using IKFast solver...")
                joint_conf = pr2_inverse_kinematics(
                    self.robot_body, 
                    arm, 
                    target_pose,
                    obstacles=[],  # Start without obstacles for initial attempt
                    custom_limits=custom_limits
                )
                
                if joint_conf is not None:
                    print(f"  IKFast solution found: {[round(j, 3) for j in joint_conf]}")
                    return joint_conf
                else:
                    print("  IKFast failed, trying PyBullet IK...")
            
            # Fallback to PyBullet IK
            print("  Using PyBullet IK solver...")
            
            # Try PyBullet's built-in IK
            import pybullet as p
            gripper_link = get_gripper_link(self.robot_body, arm)
            target_position = target_pose[0]
            target_orientation = target_pose[1]
            
            # Use PyBullet's calculateInverseKinematics with better parameters
            joint_positions = p.calculateInverseKinematics(
                self.robot_body,
                gripper_link,
                target_position,
                target_orientation,
                lowerLimits=[-3.14] * len(arm_joints),
                upperLimits=[3.14] * len(arm_joints),
                jointRanges=[6.28] * len(arm_joints),
                restPoses=default_conf,
                maxNumIterations=1000,  # Increase iterations for better accuracy
                residualThreshold=1e-5  # Lower threshold for better precision
            )
            
            if joint_positions is not None and len(joint_positions) >= len(arm_joints):
                # Extract only the arm joint positions
                arm_joint_positions = joint_positions[:len(arm_joints)]
                
                # Validate the IK solution by checking if it actually reaches the target
                # Temporarily set the joints to this configuration
                current_joints = get_joint_positions(self.robot_body, arm_joints)
                set_joint_positions(self.robot_body, arm_joints, arm_joint_positions)
                
                # Check where the gripper actually ends up
                actual_gripper_pose = get_link_pose(self.robot_body, gripper_link)
                actual_position = actual_gripper_pose[0]
                
                # Calculate position error
                position_error = ((actual_position[0] - target_position[0])**2 + 
                                 (actual_position[1] - target_position[1])**2 + 
                                 (actual_position[2] - target_position[2])**2)**0.5
                
                print(f"  PyBullet IK solution found: {[round(j, 3) for j in arm_joint_positions]}")
                print(f"  Target position: {target_position}")
                print(f"  Actual position: {actual_position}")
                print(f"  Position error: {position_error:.4f}m")
                
                # Restore original joint positions
                set_joint_positions(self.robot_body, arm_joints, current_joints)
                
                # Accept solution only if error is small enough
                if position_error < 0.05:  # 5cm tolerance
                    print(f"  IK solution validated (error: {position_error:.4f}m)")
                    return list(arm_joint_positions)
                else:
                    print(f"  IK solution rejected - too much error ({position_error:.4f}m)")
                    return None
            else:
                print("  Both IK solvers failed")
                return None
    ...
    def _execute_ik_motion(self, joint_conf, description="motion"):
        """Execute joint motion to IK solution"""
        if joint_conf is None:
            print(f"  Cannot execute {description} - no valid joint configuration")
            return False
            
        try:
            print(f"  Executing {description}...")
            set_joint_positions(self.robot_body, self.left_joints, joint_conf)
            wait_for_duration(0.8)
            
            # Update attached object position during motion (only if object is attached)
    ...
    def _attach_object_to_robot(self, pddl_obj_name):
        ...
        try:
            ...
            # Check if gripper is close enough to object for grasping
            gripper_pos = gripper_pose[0]
            object_pos = object_pose[0]
            distance = ((gripper_pos[0] - object_pos[0])**2 + 
                       (gripper_pos[1] - object_pos[1])**2 + 
                       (gripper_pos[2] - object_pos[2])**2)**0.5
            print(f"    Distance between gripper and object: {distance:.3f}m")
            
            if distance > 0.15:  # 15cm threshold
                print(f"    Warning: Gripper is {distance:.3f}m away from object - grasping may not be realistic")
            
            # Step 2: Calculate relative transform (grasp_pose) - this keeps object in place relative to gripper
            self.grasp_pose = multiply(invert(gripper_pose), object_pose)
            
            # Step 3: Store attachment info
            self.attached_object_id = body_id
            
            # Step 4: Do NOT move the object! The gripper should be at the object's location already
            # The object stays where it is, and we just track the relative transform
            print(f"    Object remains at position: {object_pos[:3]}")
            print(f"    Relative transform calculated for attachment tracking")
    ...
    def _execute_move(self, action: str, verbose: bool) -> bool:
        ...
        for waypoint in base_path:
            ...
            # Update attached object position during movement (only if object is attached)
            if self.attached_object_id is not None:
                self._update_attached_object()
    ...
    def _execute_pickup_from_region(self, action: str, verbose: bool) -> bool:
        """Execute pickup_from_region action with proper IK-based grasping"""
        parts = action.split()
        if len(parts) < 4:
            return False
        ...
        # Step 1: Get object pose and calculate optimal grasp pose
        print(f"  Step 1: Analyzing object pose for {pddl_obj}...")
        grasp_pose, object_pose = self._get_object_pose_for_grasping(pddl_obj)
        if grasp_pose is None:
            print(f"  Failed to get object pose for {pddl_obj}")
            return False
        
        # Step 2: Calculate approach pose (slightly above grasp pose)
        approach_position = (
            grasp_pose[0][0],
            grasp_pose[0][1], 
            grasp_pose[0][2] + 0.10  # 10cm higher for approach (was 15cm, reducing for reachability)
        )
        approach_pose = (approach_position, grasp_pose[1])
        
        # Step 3: Calculate IK for approach pose
        print(f"  Step 2: Calculating IK for approach pose...")
        approach_conf = self._calculate_inverse_kinematics(approach_pose, arm='left')
        if approach_conf is None:
            print(f"  IK failed for approach pose - using fallback motion")
            self._stretch_arm_to_region(region)
        else:
            print(f"  Step 3: Moving to approach pose...")
            self._execute_ik_motion(approach_conf, "approach motion")
        
        # Step 4: Calculate IK for grasp pose
        print(f"  Step 4: Calculating IK for grasp pose...")
        grasp_conf = self._calculate_inverse_kinematics(grasp_pose, arm='left')
        if grasp_conf is None:
            print(f"  IK failed for grasp pose - using approximate grasp motion")
            # Fallback to approximate grasping motion
            grasp_pose_fallback = [0.3, 0.1, -0.2, -1.4, 0.0, -1.0, 0.0]
            set_joint_positions(self.robot_body, self.left_joints, grasp_pose_fallback)
            wait_for_duration(0.5)
        else:
            print(f"  Step 5: Moving to grasp pose...")
            self._execute_ik_motion(grasp_conf, "grasp motion")
        
        # Step 6: Simulate gripper closing and attach object
        print(f"  Step 6: Grasping {pddl_obj}...")
        ...
        # Step 7: Calculate retreat pose (back to approach pose)
        print(f"  Step 7: Retreating with {pddl_obj}...")
        if approach_conf is not None:
            self._execute_ik_motion(approach_conf, "retreat motion")
        else:
            # Fallback retreat motion
            set_joint_positions(self.robot_body, self.left_joints, TOP_HOLDING_LEFT_ARM)
            wait_for_duration(0.5)
        
        print(f"✅ Successfully picked up {pddl_obj} from {region} using IK-based grasping")
        return True
    
    def _execute_pickup_from_container(self, action: str, verbose: bool) -> bool:
        """Execute pickup_from_container action with proper IK-based grasping"""
        parts = action.split()
        if len(parts) < 4:
            return False
        ...
        # Step 1: Get object pose and calculate optimal grasp pose
        print(f"\nStep 1: Analyzing object pose for {pddl_obj}...")
        grasp_pose, object_pose = self._get_object_pose_for_grasping(pddl_obj)
        if grasp_pose is None:
            print(f"  Failed to get object pose for {pddl_obj}")
            return False
        ...
        # Step 3: Calculate IK for approach pose
        print(f"\nStep 2: Calculating IK for approach pose...")
        approach_conf = self._calculate_inverse_kinematics(approach_pose, arm='left')
        if approach_conf is None:
            print(f"  IK failed for approach pose - using fallback motion")
            self._stretch_arm_to_container(container)
        else:
            print(f"\nStep 3: Moving to approach pose...")
            self._execute_ik_motion(approach_conf, "approach motion")
  

        # Step 4: Calculate IK for grasp pose
        print(f"\nStep 4: Calculating IK for grasp pose...")
        grasp_conf = self._calculate_inverse_kinematics(grasp_pose, arm='left')
        if grasp_conf is None:
            print(f"  IK failed for grasp pose - using approximate grasp motion")
            # Fallback to approximate grasping motion
            ...
            print(f"\nStep 5: Moving to grasp pose...")
            self._execute_ik_motion(grasp_conf, "grasp motion")



        # Step 6: Attach object to robot gripper at grasp pose
        print(f"\nStep 6: Attaching {pddl_obj} to gripper at grasp pose...")
        if self._attach_object_to_robot(pddl_obj):
            self.holding_object = pddl_obj  # Store PDDL name
        else:
            return False
        
        # Step 7: Close gripper after object is attached
        print(f"Step 7: Closing gripper to secure {pddl_obj}...")
        close_gripper(self.robot_body, 'left')
        wait_for_duration(0.5)
            
        # Step 8: Calculate retreat pose (back to approach pose)
        print(f"\nStep 8: Retreating with {pddl_obj}...")
        if approach_conf is not None:
            self._execute_ik_motion(approach_conf, "retreat motion")
        else:
            # Fallback retreat motion
        ...
        # Step 9: Move to navigation pose for easier robot movement
        print(f"Step 9: Moving to navigation pose...")
        set_joint_positions(self.robot_body, self.left_joints, SIDE_HOLDING_LEFT_ARM)
        wait_for_duration(0.5)
        
        print(f"✅ Successfully picked up {pddl_obj} from {container} using IK-based grasping")
        return True
...
def main():
    ...
    #     "pickup_from_container pr2 potato fridge",
    #     # "pickup_from_region pr2 plate countertop_1",
    #     "move pr2 fridge countertop_1",
    # ]

    test_actions = [
        "move pr2 kitchen_corner cabinet",
        "pickup_from_container pr2 vinegar cabinet",
        # "pickup_from_region pr2 plate countertop_1",
        "move pr2 cabinet countertop_1",
    ]
    
    try:
        print("=== TESTING SIMULATOR EXECUTOR V6 (WITH IMPROVED IK GRASPING) ===")
        print("Testing improved pickup_from_container with proper inverse kinematics...")
