...
def main():
    ...
    for i, (subtask, goal) in enumerate(zip(subtasks, pddl_goals)):  # Show all subtasks
        print(f"Subtask {i+1}: {subtask}")
        print(f"PDDL Goal {i+1}: {goal}")
        print()
    
    # Step 4: Solve all subtasks
    current_init_state = None
    all_action_plans = []
    
    for i in range(len(subtasks)):  # Solve all subtasks
        subtask_num = i + 1
        subtask = subtasks[i]
    ...
    total_actions = 0
    summary_content = []
    summary_content.append(f"Task: {task_instructions}")
    ...
    with open(config_file, 'w') as f:
        ...
        f.write(f"Task: {task_instructions}\n")
